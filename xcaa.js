...
//>>>>>>>>>[ KHUSUS INFO BOT ]<<<<<<<<<<\\
         ...
         case 'creator':
              ...
              const vname_ = xcaa.contacts[i] != undefined ? xcaa.contacts[i].vname || xcaa.contacts[i].notify : undefined
              ini_list.push({
              "displayName": 'Owner Xcaa',
              "vcard": `BEGIN:VCARD\nVERSION:3.0\nN:Sy;Bot;;;\nFN:${vname_ ? `${vname_}` : `${xcaa.user.name}`}\nORG: `No Spam!`;\nitem1.TEL;waid=${i.split('@')[0]}:${i.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`
              })
              }
              xcaa.sendMessage(from, { "displayName": `${ini_list.length} kontak`,"contacts": ini_list }, 'contactsArrayMessage', {quoted: ftroli, contextInfo: {"mentionedJid": members_ids}})
              break
         case 'info':
              if (isBanned) return sendButMessage (from, banned1, banned2, banned3, {quoted: ftroli})
              me = xcaa.user
